...
CMesh * CMesh::CreateFromContainer(CFBXLoader & _loader, int index)
{
	...
	for (UINT i = 0; i < vecBone.size(); ++i)
	{
		...
		for (UINT j = 0; j < vecBone[i]->vecKeyFrame.size(); ++j)
		{
			...
			tKeyframe.fTime = vecBone[i]->vecKeyFrame[j].dTime;
			tKeyframe.iFrame = j;
			tKeyframe.vTranslate.x = vecBone[i]->vecKeyFrame[j].matTransform.GetT().mData[0];
			tKeyframe.vTranslate.y = vecBone[i]->vecKeyFrame[j].matTransform.GetT().mData[1];
			tKeyframe.vTranslate.z = vecBone[i]->vecKeyFrame[j].matTransform.GetT().mData[2];

			tKeyframe.vScale.x = vecBone[i]->vecKeyFrame[j].matTransform.GetS().mData[0];
			tKeyframe.vScale.y = vecBone[i]->vecKeyFrame[j].matTransform.GetS().mData[1];
			tKeyframe.vScale.z = vecBone[i]->vecKeyFrame[j].matTransform.GetS().mData[2];

			tKeyframe.qRot.x = vecBone[i]->vecKeyFrame[j].matTransform.GetQ().mData[0];
			tKeyframe.qRot.y = vecBone[i]->vecKeyFrame[j].matTransform.GetQ().mData[1];
			tKeyframe.qRot.z = vecBone[i]->vecKeyFrame[j].matTransform.GetQ().mData[2];
			tKeyframe.qRot.w = vecBone[i]->vecKeyFrame[j].matTransform.GetQ().mData[3];
	...
	for (UINT i = 0; i < vecAnimClip.size(); ++i)
	{
		...
		tClip.iStartFrame = vecAnimClip[i]->tStartTime.GetFrameCount(vecAnimClip[i]->eMode);
		tClip.iEndFrame = vecAnimClip[i]->tEndTime.GetFrameCount(vecAnimClip[i]->eMode);
